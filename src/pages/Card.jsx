...
const getMsg = (name) => NAME_MESSAGES[(name || '').trim().toLowerCase()] || DEFAULT_MSG
...
export default function Card() {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            link.download = `womens-day-${name}.png`
            link.href = canvas.toDataURL('image/png')
            link.click()
        } catch (err) {
            console.error('Download error:', err)
            alert('Could not download card. Try right-clicking the card image to save it.')
        } finally {
        ...
        <div className="page" style={{ zIndex: 1, paddingBottom: '3rem' }}>
            {confetti && (
                <ReactConfetti
                    ...
                    height={winSize.h}
                    numberOfPieces={230}
                    colors={['#ff80ab', '#ce93d8', '#f48fb1', '#e91e63', '#ab47bc', '#fff', '#ffcdd2']}
                    recycle={false}
                    gravity={0.18}
            ...
            <div className="glass-card" style={{ maxWidth: 420 }}>
                ...
                <div className="gap-row">
                    ...
                    <button
                        ...
                        onClick={() => navigate('/feedback', { state: { submissionId, name } })}
                        id="feedback-btn"
                    >
                        Next ➡️
                    </button>
